...
int main(int argc, const char *argv[])
{
    ...
    int errors = 0;
    float macs =
        2.0 * float(heads) * float(S_q) * float(S_kv) * float(d) + 8.0 * float(heads) * float(S_q) * float(S_kv);
    ...
    if (trace_size > 0) {
        matmul_common::write_out_trace((char *)bufTrace, trace_size, vm["trace_file"].as<std::string>());
    }

    std::cout << std::endl << "Avg NPU MHA time: " << npu_time_total / n_iterations << "us." << std::endl;
    std::cout << "Avg NPU gflops: " << macs / (1000 * npu_time_total / n_iterations) << std::endl;

    std::cout << std::endl << "Min NPU MHA time: " << npu_time_min << "us." << std::endl;
