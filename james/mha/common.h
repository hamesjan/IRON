...
#define print_row(what)                                                                                                \
    ...
    int col = n_cols - n_printable_cols / 2;
...
template <>
void print_matrix(const std::vector<int8_t> matrix,
                  int n_cols,
                  int n_printable_rows,
                  int n_printable_cols,
                  std::ostream &ostream,
                  const char col_sep[],
                  const char elide_sym[],
                  int w)
{
    ...
    print_matrix(cast_matrix, n_cols, n_printable_rows, n_printable_cols, ostream, col_sep, elide_sym, w);
}

constexpr int max_printable_errors = 32;

template <typename Tout> struct error {
    int head;
    int row;
    int col;
    Tout expected;
...
verify_single(int head, int row, int col, Tout expected, Tout actual, float abs_tol, float rel_tol) {
    ...
    if (!match) {
        return error<Tout>{head, row, col, expected, actual};
    }
    return std::nullopt;
}

template <typename Tout>
void print_error_summary(std::ostream &os,
                         int n_errors,
                         int total_elements,
                         ...
                         Tout max_rel_error)
{
    ...
    for (struct error<Tout> &err : errors) {
        os << "[" << std::setw(5) << err.head << ", " << std::setw(5) << err.row << ", " << std::setw(5) << err.col
           << "] " << std::setw(4) << std::setprecision(2) << std::fixed << (float)err.actual << " =!= " << std::setw(4)
           << std::setprecision(2) << std::fixed << (float)err.expected << std::endl;
    }

    if (n_errors > max_printable_errors) {
        os << "...and " << std::setw(0) << n_errors - max_printable_errors << " further errors." << std::endl;
    }

    if (n_errors > 0) {
        os << "Maximum relative error: " << std::setw(3) << std::setprecision(0) << max_rel_error * 100 << "%"
           << std::endl;
    }
}
...
{
    os << "\r" << std::string((int)(progress * len), '|') << std::string(len - (int)(progress * len), ' ')
       << std::setw(4) << std::fixed << std::setprecision(0) << progress * 100 << "%" << "\r";
}
...
void write_out_trace(char *traceOutPtr, size_t trace_size, std::string path)
{
    ...
    for (uint i = 0; i < trace_size / sizeof(traceOut[0]); i++) {
